...
function type2secretPrompt(type) {
  ...
  switch (type) {
    ...
    case 50:
      return '按照如下格式输入: AccessKey|SecretKey, 如果上游是API Plus，则直接输ApiKey';
    case 51:
...
const EditChannelModal = (props) => {
  ...
  const handleInputChange = (name, value) => {
    ...
    if (name === 'base_url' && value.endsWith('/v1')) {
      Modal.confirm({
        title: '警告',
        content:
          '不需要在末尾加/v1，API Plus会自动处理，添加后可能导致请求失败，是否继续？',
        onOk: () => {
  ...
  const submit = async () => {
    ...
    if (hasModelMapping) {
      ...
      parsedModelMapping = normalizeModelMappingConfig(parsedModelMapping);
      const modelMappingValidationError =
        validateModelMappingConfig(parsedModelMapping);
      if (modelMappingValidationError) {
        showInfo(t(modelMappingValidationError));
        return;
      }
      localInputs.model_mapping = JSON.stringify(parsedModelMapping, null, 2);
    }
                      ...
                      {inputs.type === 8 && (
                        <>
                          <Banner
                            type='warning'
                            description={t(
                              '如果你对接的是上游One API或者API Plus等转发项目，请使用OpenAI类型，不要使用此类型，除非你知道你在做什么。',
                            )}
                            className='!rounded-lg'
                          />
                          <div>
                ...
                <div ref={(el) => (formSectionRefs.current.modelConfig = el)}>
                  <Card className='!rounded-2xl shadow-sm border-0 mb-6'>
                    ...
                    <JSONEditor
                      ...
                      placeholder={
                        t(
                          '此项可选，用于修改请求体中的模型名称。值既可以是目标模型字符串，也可以是包含 target_model 和可选 endpoint_type 的对象，例如：',
                        ) +
